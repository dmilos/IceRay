...
I_picture ={}
I_picture[ 'width']  = int( 800*0.2 )
I_picture['height']  = int( 600*0.2 )
...
I_scene['pigment']    =  'T-B-refract-schlick' #'T-0-reflect-mirror'
...
I_config['decoration']   = {}
I_config['decoration']['size']   = IceRayPy.type.math.coord.Scalar3D(   1.1/3, 1.1/3, 0.01 )
I_config['decoration']['center'] = IceRayPy.type.math.coord.Scalar3D( 1, 1, 1 )
I_config['decoration']['color'] = IceRayPy.type.color.RGB( 135/10.0, 135/10.0, 135/10.0 )
...
I_config['camera']['view']   = IceRayPy.type.math.coord.Scalar3D( 0, 0, 0 )
for index in range( 500, 501, 6 ):
    angle = 150    
    I_config['camera'][ 'eye']   = IceRayPy.type.math.coord.Scalar3D( 2.6 * math.cos( math.radians( angle ) ), 2.6 * math.sin( math.radians( angle ) ) ,  +g )
    for jitter in [ 'none' ]:  # , 'vdc', 'triangle', 'sobol', 'random', 'sunflower', 'congruent', 'grid', 'hexagon' 
        for object in [ 'Q-sphere'  ]:  #'Q-sphere' 'V-vacuum', 'Q-sphere', 'Q-util-cylinder', 
            ...
            for blossom in [ 'congruent', 'kmeans', 'triangle', 'sunflower', 'grid', 'hexagon', 'vdc', 'sobol', 'random'  ]: #  
                I_picture['index'] = index
                I_picture['prefix'] = "%03i"%(index) + '_' + blossom + "_"
                I_config['room']['radiosity']['patch']  = math.radians( 4 )/index #<! Used also for sample number calculation
                I_config['room']['radiosity']['jitter-type' ]   = jitter
                I_config['room']['radiosity']['jitter-angle' ]   = 0.5 * I_config['room']['radiosity']['patch']
                I_config['room']['radiosity']['angle'  ] = math.radians( 60 )
