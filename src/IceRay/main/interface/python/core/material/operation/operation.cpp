...
void expose_IceRay_material_compute_operation_mapping()
 {
  ...
  typedef  GS_DDMRM::S_IceRay::S_material::S_compute::S_mapping::GT_identical    GTs_identical;
  typedef boost::python::class_< GTs_identical, boost::python::bases< GTs_instruction > > ( "MaterialCoord3DMappingIdentical" )
    .def( boost::python::init<>() )
    .def( boost::python::init< GTs_size,GTs_size>( ) )
    ;
