           ...
           class GC_approx // http://radsite.lbl.gov/radiance/papers/sg92/page4.gif  page 268, 5b
            {
             ...
             public:
               GC_approx( T_coord const&  P_direction, T_color const&  P_specular, T_color const&  P_alphaX, T_color const&  P_alphaY )
             ...
             public:
               bool F_process
                ...
                )
                {
                 ...
                 T_scalar I_c0 = sqrt( I_ln * I_vn ) * 4 * Is_phi;
                 T_scalar I_e1 = -2/( 1 + I_hn );

                 P_result.set( 0, P_light[0] * M2_specular[0] * ( T_scalar(1)/( I_c0 * M2_alphaX[0] * M2_alphaY[0] ) ) * exp( I_e1 * ( I_hx / M2_alphaX[0]/ M2_alphaX[0] + I_hy / M2_alphaY[0]/ M2_alphaY[0] ) ) );
                 P_result.set( 1, P_light[1] * M2_specular[1] * ( T_scalar(1)/( I_c0 * M2_alphaX[1] * M2_alphaY[1] ) ) * exp( I_e1 * ( I_hx / M2_alphaX[1]/ M2_alphaX[1] + I_hy / M2_alphaY[1]/ M2_alphaY[1] ) ) );
                 P_result.set( 2, P_light[2] * M2_specular[2] * ( T_scalar(1)/( I_c0 * M2_alphaX[2] * M2_alphaY[2] ) ) * exp( I_e1 * ( I_hx / M2_alphaX[2]/ M2_alphaX[2] + I_hy / M2_alphaY[2]/ M2_alphaY[2] ) ) );
