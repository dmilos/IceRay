          ...
          //: public S_IceRay::S_material::S_illumination::GC__pure
            {
             ...
             public:
               GC_f30( T_color const& P_rho, T_color const& P_sigma )
                {
                 M2_a[0] = Fs_a( P_rho[0], P_sigma[0] ); M2_b[0] = Fs_a( P_rho[0], P_sigma[0] );
                 M2_a[1] = Fs_a( P_rho[1], P_sigma[1] ); M2_b[1] = Fs_a( P_rho[0], P_sigma[0] );
                 M2_a[2] = Fs_a( P_rho[2], P_sigma[2] ); M2_b[2] = Fs_a( P_rho[0], P_sigma[0] );
                }
               ...
               bool F_process
                (
                 ...
                 { // Tested with succes
                  ...
                  T_scalar I_alpha = std::max( I_thetaR, I_thetaI );
                  T_scalar I_beta  = std::min( I_thetaR, I_thetaI );
                  ...
                  P_result.set( 0, P_energy[0] * I_cos_thetaI * ( M2_a[0] + M2_b[0] * I_cos_phi * sin(I_alpha) * tan(I_beta) ) );
                  P_result.set( 1, P_energy[1] * I_cos_thetaI * ( M2_a[1] + M2_b[1] * I_cos_phi * sin(I_alpha) * tan(I_beta) ) );
                  P_result.set( 2, P_energy[2] * I_cos_thetaI * ( M2_a[2] + M2_b[2] * I_cos_phi * sin(I_alpha) * tan(I_beta) ) );
             ...
             public:
              static T_scalar Fs_A( T_scalar const& P_rho )
                {
                 T_scalar Ir_result; 
                 Ir_result = T_scalar(1) -  P_rho*P_rho/( P_rho*P_rho + 0.33 ) / T_scalar( 2 );
                 return Ir_result;
                }
              static T_scalar Fs_B( T_scalar const& P_rho )
               {
                T_scalar Ir_result; 
                Ir_result = T_scalar(0.45) * P_rho*P_rho/( P_rho*P_rho + 0.09 );
                return Ir_result;
               }

              static T_scalar Fs_a( T_scalar const& P_rho, T_scalar const& P_sigma )
                {
                 return Fs_A(P_rho) * P_sigma / ::math::constants::PHI;
                }
              static T_scalar Fs_b( T_scalar const& P_rho, T_scalar const& P_sigma )
               {
                 return Fs_B(P_rho) * P_sigma / ::math::constants::PHI;
               }
