#ifndef Dh_DDMRM_Iceray_material_compute_illumination_Burley_specular_HPP_
 #define Dh_DDMRM_Iceray_material_compute_illumination_Burley_specular_HPP_

 #include <tuple>
 #include <limits>

 #include "../../illumination/burley/specular.hpp"
             ...
             class GC_specular
              : public GS_DDMRM::S_IceRay::S_material::S_compute::GC_instruction
              {
               public:
                 ...
                 enum Ee_input
                  {
                   En_inColor_Scaling = 0, 
                   En_inScalar_Roughness = 0, En_inScalar_Gamma = 1, En_inScalar_F0 = 2,
               ...
               public:
                 GC_specular
                  (
                   ...
                   ,T_size const& P_inSize_SpotBegin   
                   ,T_size const& P_inSize_SpotEnd     
                   ,T_size const& P_specular   
                   ,T_size const& P_roughness  
                   ,T_size const& P_gamma      
                   ,T_size const& P_F0         
                  )
                  {
                   ...
                   F_input<T_color>(    En_inColor_Scaling,     P_specular );
                   F_input<T_scalar>(   En_inScalar_Roughness,  P_roughness );
                   F_input<T_scalar>(   En_inScalar_Gamma,      P_gamma     );
                   F_input<T_scalar>(   En_inScalar_F0,         P_F0        );
                 ...
                 bool    Fv_execute( T_beam &P_next, T_pigment::T_intersect const& P_intersect, T_state const& P_state )const
                  {
                   ...
                   T_color const& I_specular   = M2_memoryColor->Fv_load(  F_input<T_color>( En_inColor_Scaling  ) );
                   T_scalar const& I_roughness = M2_memoryScalar->Fv_load( F_input<T_scalar>( En_inScalar_Roughness ) );
                   T_scalar const& I_gamma     = M2_memoryScalar->Fv_load( F_input<T_scalar>( En_inScalar_Gamma ) );
                   T_scalar const& I_F0        = M2_memoryScalar->Fv_load( F_input<T_scalar>( En_inScalar_F0 ) );

                   GS_DDMRM::S_IceRay::S_material::S_illumination::S_burley::GC_specular I_burley( I_specular, I_roughness, I_gamma, I_F0 );
                   ...
                   for( T_size I_spotIndex = I_spotBegin; I_spotIndex < I_spotEnd; ++I_spotIndex )
                    {
                     ...
                     ::math::linear::vector::addition( I_half, I_2light, I_2viewer );
                     ::math::linear::vector::length( I_half, T_scalar(1) );

                     if( true == I_burley.F_process( I_color, I_energy, I_normal, I_2viewer, I_2light, I_half ) )
                      {
                       I_summae += I_color;
                      }
                    }
