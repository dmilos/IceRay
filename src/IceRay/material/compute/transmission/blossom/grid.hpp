               ...
               public:
                 ...
                 (
                  ...
                  )const
                  {
                   ...
                   T_coord I_direction;
                   for( T_size I_v=0; I_v < P_size; ++I_v )
                   for( T_size I_u=0; I_u < P_size; ++I_u )
                    {
                     T_coord2D I_disc2d;
                     ...
                     T_scalar I_height = sqrt( T_scalar( 1 ) - I_radius * I_length2 );
                     ...
                     {
                      P_next.Fv_push();
                      auto & I_ray = P_next.Fv_top();  ++I_total;
                  ...
                  for( T_size I_index =0; I_index < I_total; ++I_index )
                   {
                    auto & I_ray = P_next.Fv_expose( I_begin + I_index );
                    I_ray.M_intesity /= T_scalar( I_total );
                    I_ray.M_coefficient = T_scalar(1)/T_scalar( I_total );
                   }
