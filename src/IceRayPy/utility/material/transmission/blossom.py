...
# todo    if( 'angle' in P_config ):
# todo        P_surface.append( IceRayPy.core.material.instruction.constant.Scalar( P_dll, P_config['angle'], I_jitter ) )
# todo
# todo    if( 'type' in P_config ):
# todo        if( 'random' == P_config['type'] ):
...
G_countRandom   =  6 #1,   2,   12,  33, 74
G_countCongruent=  6 #1,   2,   12,  33, 74
G_countSobol    =  6 #1,   2,   12,  33, 74
G_countTriangle =  6 #1,   1,    2,   5,  9,  17
G_countVDC      =  6 #1,   2,   12,  33, 74
...
def Grid(
    ...
    if( 'correction' in P_config ):
        if( 'rays' in P_config['correction'] ):
            ...
            if( 'none' == P_config['correction']['rays'] ):
                pass
